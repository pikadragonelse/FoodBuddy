...
// Import services
import { getSmartFoodSuggestions, SmartFoodSuggestion } from '@/services/foodService';
...
const SCENARIOS: Scenario[] = [
  { id: '1', emoji: '💔', title: 'Đang thất tình', description: 'Cần món gì đó an ủi tâm hồn' },
  { id: '2', emoji: '💸', title: 'Mới lãnh lương', description: 'Tự thưởng bản thân, không lo về giá' },
  { id: '3', emoji: '🏃', title: 'Muốn đi trốn', description: 'Tìm góc quán yên bình, ít người' },
  { id: '4', emoji: '🌹', title: 'Hẹn hò lãng mạn', description: 'Không gian chill, đồ ăn tinh tế' },
  { id: '5', emoji: '🥴', title: 'Giải rượu gấp', description: 'Món nước nóng hổi cho tỉnh táo' },
  { id: '6', emoji: '🥗', title: 'Eat Clean', description: 'Healthy balance, không dầu mỡ' },
];
...
export default function HomeScreen() {
  ...
  const [matchedItem, setMatchedItem] = useState<SmartFoodSuggestion | null>(null);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      Alert.alert('Lỗi 🤖', error.message || 'Không thể tìm quán. Thử lại sau nhé!');
      setStage('input');
    }
  };
    ...
    <SafeAreaView className="flex-1 bg-white" edges={['top', 'left', 'right']}>
        ...
        <View className="flex-1">
          <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
            <View className="px-6 pt-8 pb-4">
              <Text className="text-4xl font-bold text-gray-800 mb-2">Food Blind Date 🫣</Text>
              <Text className="text-lg text-gray-600 mb-4">Chọn tâm trạng, AI sẽ dẫn lối...</Text>
              <View className="bg-green-100 px-4 py-2 rounded-full self-start">
                <Text className="text-green-600 font-semibold">📍 {userLocation?.address || 'Đang lấy vị trí...'}</Text>
              </View>
            </View>

            <View className="px-6 pb-32">
              <Text className="text-xl font-bold text-gray-800 mb-4">Bạn đang thế nào?</Text>
              <View className="gap-3">
                {SCENARIOS.map((scenario) => {
                  ...
                  return (
                    <TouchableOpacity
                      key={scenario.id}
                      onPress={() => setSelectedScenario(isSelected ? null : scenario)}
                      activeOpacity={0.7}
                      className={`flex-row items-center p-4 rounded-2xl border-2 ${
                        isSelected ? 'border-[#FF6B00] bg-orange-50' : 'border-gray-100 bg-white'
                      }`}
                    >
                      ...
                      <View className="flex-1">
                        <Text className={`font-bold text-lg ${isSelected ? 'text-[#FF6B00]' : 'text-gray-800'}`}>
                          {scenario.title}
                        </Text>
          ...
          <View className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-6 py-4">
            <TouchableOpacity
              ...
              className={`rounded-2xl py-4 px-6 shadow-lg ${
                !selectedScenario || !userLocation ? 'bg-gray-300' : 'bg-[#FF6B00]'
              }`}
            >
      ...
      {stage === 'loading' && (
        <View className="flex-1 justify-center items-center px-6 bg-white">
          ...
          <Text className="text-sm text-gray-400 mt-4">AI đang nấu data, chờ chút nha... 🍳</Text>
        </View>
      )}
              ...
              >
                  ...
                  <View className="px-6 pb-6">
                      ...
                      <View className="items-center -mt-10 mb-6">
                           <View className="bg-white p-2 rounded-full shadow-xl">
                               <View className="bg-[#FF6B00] px-8 py-3 rounded-full">
                                   <Text className="text-white font-extrabold text-sm uppercase tracking-widest">FOOD MATCH 🔥</Text>
                               </View>
                           </View>
                      </View>
                      ...
                      <View className="bg-orange-50/50 p-6 rounded-[32px] mb-8 border border-orange-100">
                           <View className="flex-row items-start mb-6">
                              ...
                              <View className="flex-1 pt-1">
                                <Text className="text-gray-400 text-[10px] uppercase font-black tracking-widest mb-1">Địa chỉ</Text>
                                <Text className="text-gray-800 font-bold text-base leading-snug">
                                    {matchedItem?.address}
                                </Text>
                                {matchedItem?.distance !== -1 && (
                                  <Text className="text-[#FF6B00] font-bold text-xs mt-1">Cách bạn {formatDistance(matchedItem?.distance || 0)}</Text>
                                )}
                              </View>
                           </View>
                           ...
                           <View className="flex-row items-start">
                              ...
                              <View className="flex-1 pt-1">
                                <Text className="text-gray-400 text-[10px] uppercase font-black tracking-widest mb-1">Gợi ý trải nghiệm</Text>
                      ...
                      <View className="gap-4">
                           ...
                           >
                              <Text className="text-gray-400 font-bold text-base">Hẹn hò tiếp thôi 💘</Text>
                           </TouchableOpacity>
                      </View>
                  </View>
              </ScrollView>
